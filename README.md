...
- **シンプルな状態表示**: 安全・警告・危険の3段階を大きな文字と色で表示
...
|---|---|---|---|
| **安全** | 内側、境界まで9m以上 | 緑 | 無音 |
| **警告** | 内側、境界まで9m以下 | オレンジ | 中速ビープ(0.7秒間隔) |
| **危険** | 内側、境界まで3m以下 | 赤(全画面) | 連続アラート音 |
...
```javascript
const WARNING_M = 9;            // 内側で「警告」状態に変わる距離(m)
const DANGER_INSIDE_M = 3;      // 内側で「危険」状態に変わる距離(m)
const ACCURACY_WARN_M = 20;     // GPS精度低下を通知する閾値(m)

const BEEP_FREQUENCIES = {
  warning: 880,                 // 警告時のビープ音周波数(Hz)
  danger: 1320,                 // 危険時のビープ音周波数(Hz)
};
```
